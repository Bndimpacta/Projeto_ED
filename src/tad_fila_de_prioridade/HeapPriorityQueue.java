...
import tad_mapa.PriorityQueue;

import java.util.Comparator;

import tad_mapa.InvalidKeyException;
...
public class HeapPriorityQueue<K, V> implements PriorityQueue<K, V> {
	...
	heap = (CompleteBinaryTree<Entry<K, V>>) new ArrayListCompleteBinaryTree<Entry<K, V>>(); // Usa o ArrayListCompleteBinaryTree
	...
	heap = (CompleteBinaryTree<Entry<K, V>>) new ArrayListCompleteBinaryTree<Entry<K, V>>();
	...
	public tad_mapa.Entry<K, V> min() throws EmptyPriorityQueueException {

	if (isEmpty()) throw new EmptyPriorityQueueException("Priority queue is empty");

	return (tad_mapa.Entry<K, V>) heap.root().element();

	}

	// Insere um par chave-valor e retorna uma entrada criada

	public tad_mapa.Entry<K, V> insert(K k, V x) throws InvalidKeyException {
	...
	return (tad_mapa.Entry<K, V>) entry;

	}

	// Remove e retona uma entrada com a chave mínima

	public tad_mapa.Entry<K, V> removeMin() throws EmptyPriorityQueueException {
